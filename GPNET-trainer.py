...
optimizer=optim.Adam(filter(lambda p: p.requires_grad, model.parameters()),weight_decay=0.00452,lr=0.0100)
criterion=nn.BCELoss()

with open ('./transfer_model/freeze_conv4/minibatch8_best/result/trainres.txt',mode='w',encoding="utf-8") as h:
	print('Epoch\tLoss\tAUC',file=h)
with open ('./transfer_model/freeze_conv4/minibatch8_best/result/validationres.txt',mode='w',encoding="utf-8") as f:
...
for epoch in range(60):
    ...
    if (epoch+1) %1 ==0:
        ...
        with open('./transfer_model/freeze_conv4/minibatch8_best/result/trainres.txt',mode='a',encoding="utf-8") as h:
    ...
    if (epoch+1) % 1 ==0:
        with open('./transfer_model/freeze_conv4/minibatch8_best/result/validationres.txt',mode='a',encoding="utf-8") as f:
            print(str(epoch+1)+'\t'+str(loss_validation)+'\t'+str(AUC_validation),file=f)
    if (epoch+1) % 1 ==0:
        torch.save(model.state_dict(),'./transfer_model/freeze_conv4/minibatch8_best/ref/train-{:03d}.pth'.format(epoch+1))
