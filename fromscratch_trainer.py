...
optimizer = optim.Adam(model.parameters(), weight_decay=0.00360, lr=0.00110)
criterion = nn.BCELoss()

with open('./fromscratch_model/minibatch24_best/result/trainres.txt', mode='w', encoding="utf-8") as h:
    print('Epoch\tLoss\tAUC', file=h)
with open('./fromscratch_model/minibatch24_best/validationres.txt', mode='w', encoding="utf-8") as f:
...
for epoch in range(60):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        with open('./fromscratch_model/minibatch24_best/result/trainres.txt', mode='a',
                  encoding="utf-8") as h:
    ...
    if (epoch + 1) % 1 == 0:
        with open('./fromscratch_model/minibatch24_best/result/validationres.txt', mode='a',
                  encoding="utf-8") as f:
            print(str(epoch + 1) + '\t' + str(loss_validation) + '\t' + str(AUC_validation), file=f)
    if (epoch + 1) % 1 == 0:
        torch.save(model.state_dict(),
                   './fromscratch_model/minibatch24_best/ref/train-{:03d}.pth'.format(epoch + 1))
